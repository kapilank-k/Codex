...
export default function RightPanel({ onChatComplete }) {
  ...
  const getAIResponse = (userInput) => {
    // ... (Your AI response logic remains the same)
    const responses = {
  ...
  const handleSend = (text) => {
    ...
    setTimeout(() => {
      ...
      // After the AI responds, we wait a moment and then signal that the chat is complete.
      setTimeout(() => {
        onChatComplete();
      }, 1200); // 1.2 second delay before proceeding to the next level
